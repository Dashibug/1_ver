...
function* script(r: SberRequest) {
    ...
    let phrase;
    let phraseIndex;
    phrase = `Добро пожаловать в приложение «Даши»! где ${(appeal === 'official' ? 'вам' : 'тебе')} нужно объяснить слово с экрана, используя только мимику, жесты и движения. Все слова даны на английском, надеюсь это поможет улучшить твои навыки в нем!`;
    rsp.msg = phrase;
    rsp.data = {type: 'init'};
    yield rsp;
    ...
    while (true) {
        ...
        if (r.type === 'SERVER_ACTION' && r.act?.action_id === 'help') {
            rsp.data = {type: 'help'}
            rsp.msg = 'Суть игры - объяснить слово с экрана, используя только мимику, жесты и движения. Кнопка «Новое слово»  - выдает новое слово из того же режима. Кнопка «Заново» - сбрасывает набранные очки. Удачной игры! Чтобы закрыть это окно - достаточно сказать «Закрой помощь»';
            ...
            if (command.type === 'changeword') {
                ...
                rsp.data = command;
                    ...
                    rsp.msg =  ` Добро пожаловать в приложение «Английский крокодил»! Данный смартап предназначен для всем известной игры, где тебе нужно объяснить слово с экрана, используя только мимику, жесты и движения. Здесь можно попросить новое слово, поменять режим и даже вести счёт отгаданных слов!`;
                    rsp.data = command;
            } else if (command.type === 'smartapp') {
                if (count == 0) {
                    rsp.msg = ` Добро пожаловать в приложение «Даши»! Данный смартап предназначен для всем известной игры, где тебе нужно объяснить слово с экрана, используя только мимику, жесты и движения. Здесь можно попросить новое слово, поменять режими тут даже есть счёт отгаданных слов!`;
                    rsp.data = command;
                    count+=1;
                }
            }
